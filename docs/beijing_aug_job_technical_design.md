...
## 文档信息
- **Job名称**: `signing_and_sales_incentive_aug_beijing()`
- **创建日期**: 2025-01-02
- **版本**: v1.0
- **状态**: 生产运行
...
### 1.1 功能目的
处理北京地区2025年8月的签约和销售激励活动，实现自动化的合同数据获取、奖励计算和通知发送。
...
├── 数据处理层: process_data_jun_beijing()
├── 奖励计算层: determine_rewards_jun_beijing_generic()
├── 通知发送层: notify_awards_jun_beijing()
└── 文件管理层: archive_file()
```
...
# 业务规则配置
SINGLE_PROJECT_CONTRACT_AMOUNT_LIMIT_BJ_FEB = 500000  # 工单金额上限
PERFORMANCE_AMOUNT_CAP_BJ_FEB = 500000  # 业绩金额上限
ENABLE_PERFORMANCE_AMOUNT_CAP_BJ_FEB = True  # 启用业绩金额上限
```

### 4.2 奖励配置
使用通用奖励配置系统，配置键为 "BJ-2025-08"：
```python
"BJ-2025-08": {
    "lucky_number": "8",
    "lucky_rewards": {
...
- `process_data_jun_beijing()`: 北京6月数据处理逻辑（复用）
- `determine_rewards_jun_beijing_generic()`: 通用奖励计算
- `get_unique_housekeeper_award_list()`: 获取管家历史奖励
- `notify_awards_jun_beijing()`: 北京通知发送（复用6月逻辑）
...
#### 6.2.1 原始合同数据文件 (ContractData-BJ-Aug.csv)
```csv
...
- `活动城市(province)`: 城市代码，北京为"110000" (String)
...
#### 6.2.2 业绩数据文件 (PerformanceData-BJ-Aug.csv)
```csv
...
- `奖励类型`: 奖励类型，如"幸运数字"、"节节高" (String)
- `奖励名称`: 具体奖励名称，如"接好运"、"达标奖" (String)
...
#### 6.3.2 奖励计算返回结构
```python
# determine_rewards_jun_beijing_generic() 返回值
(
    "幸运数字, 节节高",              # reward_types (String)
    "接好运, 达标奖",                # reward_names (String)
    "距离 优秀奖 还需 35000 元"      # next_reward_gap (String)
)
```

### 6.4 通知数据结构

#### 6.4.1 发送状态文件 (send_status_bj_aug.json)
```json
{
...
恭喜 【🏆精英管家】张三 签约合同 YHWX-BJ-2024080001 并完成线上收款🎉🎉🎉
...
```
达成接好运奖励条件，获得签约奖励36元 🧧🧧🧧
达成达标奖奖励条件，奖励金额 200 元，同时触发"精英连击双倍奖励"，奖励金额🚀直升至 400 元！🧧🧧🧧
```
...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| v1.0 | 2025-01-02 | 初始版本，复用6月数据处理逻辑，启用8月配置 |
