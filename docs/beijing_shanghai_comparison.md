...
1. **北京特色**：支持历史合同处理，启用工单金额上限，使用个人序号幸运数字
2. **上海特色**：支持双轨统计（平台单/自引单），无工单金额限制，奖励层级更多
3. **共同特性**：使用SQLite存储，支持项目地址去重，支持奖励历史检查

## 4. 数据处理流程差异

| 处理步骤 | 北京9月 | 上海9月 |
|----------|---------|---------|
| **数据获取** | Metabase API | Metabase API |
| **历史合同** | 获取并处理 | 跳过 |
| **金额上限** | 应用5万上限 | 不限制 |
| **订单分类** | 仅平台单 | 平台单+自引单 |
